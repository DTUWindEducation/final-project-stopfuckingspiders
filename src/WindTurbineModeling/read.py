...
def read_blade_definition(file_path: Union[str, Path]):
    """
    Reads a blade definition file and parses its content into a pandas DataFrame.
    The function expects the file to contain numeric data lines corresponding to 
    blade properties, with each line containing at least 10 numeric values. The 
    resulting DataFrame will have predefined column names.
    Parameters:
    -----------
    ...
    --------
    pd.DataFrame
        A pandas DataFrame containing the parsed blade data with the following columns:
        - "BlSpn": Blade span
