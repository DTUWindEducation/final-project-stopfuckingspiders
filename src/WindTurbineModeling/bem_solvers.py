...
class BaseBEMSolver:
    ...
    def __init__(self):
        ...
        Attributes:
            ...
            elemental_data (None): Placeholder for storing data related to
            elements.
    ...
    def load_input_data(self):
        """
        Load all required input data for the wind turbine blade element
        momentum (BEM) solver.
        This method loads and processes the following input data:
        - Blade geometry data from a specified input file,
        filtering out invalid blade IDs (BlAFID <= 1).
        - Operational settings from a specified input file,
        filtering wind speeds to the range [3.0, 25.0].
        - Airfoil geometry and aerodynamic coefficient data from specified
        files, categorized by shape and coefficient identifiers.
        The processed data is stored in the `self.inputs` dictionary with the
        following keys:
        ...
        - 'airfoils': Dictionary mapping valid blade IDs (BlAFIDs) to their
        corresponding airfoil coefficient DataFrames.
        ...
        settings = settings[(settings["WindSpeed"]
                             >= 3.0) & (settings["WindSpeed"] <= 25.0)]
    ...
    def solve_blade_element(self, V0, omega, r, pitch, beta,
                            chord, df_polar, num_blades, radius, rho,
                            max_iter, tolerance, af_id):
        """
        Solve for a single blade element using the
        ...
        --------
        dict or None
            A dictionary containing the following keys if the solution
            converges:
            - 'p_n': float, Normal force per unit length (N/m).
            - 'p_t': float, Tangential force per unit length (N/m).
            - 'elemental': dict, Detailed results for the
            blade element, including:
            ...
            Returns None if the solution does not converge or if invalid
        ...
        for _ in range(max_iter):
            ...
            a_new, a_prime_new = update_induction_factors(phi, sigma,
                                                          Cn, Ct, F, a)
            if abs(a_new - a) < tolerance and abs(a_prime_new -
                                                  a_prime) < tolerance:
                break
            a, a_prime = a_new, a_prime_new

        v_rel, p_n, p_t = calc_relative_velocity_and_forces(V0, omega, r, a,
                                                            a_prime, rho,
                                                            chord, Cn, Ct)
    ...
    def get_plot_data(self):
        ...
        dictionary, and converts units where necessary for better
        visualization.
        Returns:
            dict: A dictionary containing the following keys:
                - 'wind_speeds' (numpy.ndarray): Array of wind speeds
                (V_0) from the results.
                - 'power' (numpy.ndarray): Array of power values (P)
                in megawatts.
                - 'thrust' (numpy.ndarray): Array of thrust values (T)
                in kilonewtons.
                - 'cp' (numpy.ndarray): Array of power coefficients (C_P).
                - 'ct' (numpy.ndarray): Array of thrust coefficients (C_T).
                - 'pitch' (numpy.ndarray, optional): Array of pitch values,
                if available in the results.
                - 'rpm' (numpy.ndarray, optional): Array of RPM values,
                if available in the results.
        Raises:
            ValueError: If the solver has not been run and the
            'results' attribute is missing.
        """
    ...
    def save_results(self, results, elemental_data, airfoil_data, prefix=""):
        ...
        airfoil_data : dict
            A dictionary where keys are airfoil IDs and values are
            pandas.DataFrame
        ...
        - Saves individual airfoil data CSV files in the
        "results/airfoils" directory.
        """
...
class BEMSolver(BaseBEMSolver):
    ...
    This class provides methods to perform BEM calculations,
    ...
    def run(self):
        ...
        performing calculations, saving results,
        and preparing data for plotting.
        ...
        Returns:
            tuple: A tuple containing:
                ...
                - elemental_data (defaultdict): A dictionary containing
                    elemental data (e.g., forces, moments)
    ...
    def perform_bem_calculations(self, inputs):
        """
        Perform Blade Element Momentum (BEM) calculations
        for a range of wind speeds and operating conditions.
        Parameters:
            inputs (dict): A dictionary containing the following keys:
                - 'blade_data' (pd.DataFrame): DataFrame containing
                blade geometry
                  and aerodynamic properties (e.g., spanwise positions,
                  chord lengths).
                - 'settings' (pd.DataFrame): DataFrame containing
                operating conditions
                  (e.g., wind speed, pitch angle, rotational speed).
                - 'valid_blafids' (list): List of valid blade
                aerodynamic identifiers.
                - 'airfoils' (dict): Dictionary mapping airfoil
                identifiers to their
                  aerodynamic data.
        Returns:
            tuple: A tuple containing:
                - summary_results (list): A list of dictionaries
                summarizing the
                  results for each operating condition.
                - elemental_data (defaultdict): A defaultdict
                containing lists of elemental data for each spanwise position.
        """
        summary_results = []
    ...
    def calculate_single_condition(self, V0, pitch, omega, r_s,
                                   blade_data, valid_blafids, airfoils):
        """
        ...
        Parameters:
            ...
            airfoils (list): List of airfoil polar data corresponding to
            blade airfoil IDs.
        Returns:
            dict: A dictionary containing:
                - 'summary': A dictionary summarizing the condition results,
                    including thrust, torque, power, and performance
                    coefficients.
        ...
        for r, af_id in zip(r_s, blade_data['BlAFID']):
            beta, chord = interpolate_blade_geometry(r, blade_data['BlSpn'],
                                                     blade_data['BlTwist'],
                                                     blade_data['BlChord'])
            df_polar = airfoils[valid_blafids.index(af_id)]
            bem_result = self.solve_blade_element(V0, omega, r, pitch, beta,
                                                  chord, df_polar,
                                                  NUMBER_BLADES, R, RHO,
                                                  MAX_ITER, TOLERANCE, af_id)
        ...
        T, M, P_total, CP, CT = compute_totals_and_coefficients(dT_list,
                                                                dM_list,
                                                                omega,
                                                                r_used,
                                                                RHO, R, V0,
...
class BEMSolverOpt(BaseBEMSolver):
    ...
    def run(self):
        ...
        Input:
            None (Relies on instance attributes and methods to load input
        ...
        summary_results, \
        elemental_data = self.perform_optimal_calculations(inputs)

        print("4. Saving results...")
        self.save_results(summary_results, elemental_data,
                          inputs['airfoils'], "optimal_")

        print("5. Preparing data for plotting...")

    def setup_optimal_strategy(self, inputs):
        """
        Sets up the optimal operational strategy for the wind turbine
        by interpolating
        ...
        inputs : dict
            ...
            - 'settings': A pandas DataFrame with columns ['WindSpeed',
            'PitchAngle', 'RotSpeed'].
        ...
        rpm_interp : scipy.interpolate.interp1d
            Interpolation function for rotational speed (RPM) based
            on wind speed.
        min_ws : float
        ...
        """
        operational_data = inputs['settings'][['WindSpeed', 'PitchAngle',
                                               'RotSpeed']].values
        wind_speeds = operational_data[:, 0]
        pitches = savgol_filter(operational_data[:, 1], 5, 2)
        rpms = operational_data[:, 2]

        self.pitch_interp = interp1d(wind_speeds, pitches, kind='linear',
    ...
    def generate_wind_speed_range(self):
        """
        Generates a range of wind speeds by concatenating three linearly
        spaced arrays.
        Returns:
            numpy.ndarray: A concatenated array of wind speeds covering the
            specified ranges.
        """
    ...
    def perform_optimal_calculations(self, inputs):
        """
        Perform optimal calculations for a range of wind speeds using
        optimal control.
        Parameters:
        -----------
        inputs : dict
            A dictionary containing the following keys:
            - 'blade_data': DataFrame containing blade geometry and
        ...
        tuple
            A tuple containing:
            - summary_results (list): A list of dictionaries summarizing
            the results for each wind speed.
              Each dictionary contains keys such as 'RPM', 'Pitch', and
              other summary metrics.
            - elemental_data (defaultdict): A defaultdict containing lists
            of elemental data
              for each wind speed. Keys correspond to data types.
        """
        summary_results = []
        elemental_data = defaultdict(list)
        ...
        for i, V0 in enumerate(wind_speed_range, start=1):
            print(f"   → [{i}/{len(wind_speed_range)}] \
                  Solving for V₀ = {V0:.2f} m/s using optimal control...")
    ...
    def calculate_single_condition(self, V0, pitch, omega, r_s,
                                   blade_data, valid_blafids, airfoils):
        """
        Calculate aerodynamic performance for a single operating
        ...
        dict
            ...
            - 'elemental': defaultdict of lists containing elemental
            aerodynamic data along the blade span.
        """
        elemental = defaultdict(list)
        dT_list, dM_list, r_used = [], [], []
